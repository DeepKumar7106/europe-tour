...
export default function Activities() {
    ...
    useEffect(()=> {
        ...
        const elementHeight = element.offsetHeight - (element.offsetHeight * 0.3)
        const sectionStartPoint = rect.top + window.scrollY;
        const sectionEndPoint = sectionStartPoint + elementHeight
        console.log(`The section begins at pixel: ${sectionEndPoint}`);
    ...
    const activities = experience.map((activity, index) => {
        const value = Math.floor(((index) * 100) / 16 ) + 1 
        const rangeValueEntry = value === 1 ? 0 : value
        const rangeValueExit = Math.floor(((index + 1) * 100) / 16 )
